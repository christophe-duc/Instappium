...
class AppiumUserActions:
    ...
    def get_userdata(self):
        ...
        if len(elem) == 0:
            elem = self.driver.find_elements_by_id(
                xpath.read_xpath("profile", "username_back")
            )

        username = elem[0].text
        posts = _cleanup_count(
            self.driver.find_elements_by_id(xpath.read_xpath("profile", "posts"))[
                0
            ].text
        )
        followers = _cleanup_count(
            self.driver.find_elements_by_id(xpath.read_xpath("profile", "followers"))[
                0
            ].text
        )
        following = _cleanup_count(
            self.driver.find_elements_by_id(xpath.read_xpath("profile", "following"))[
                0
            ].text
        )
        print("following={}".format(following))
        full_name = self.driver.find_elements_by_id(
            xpath.read_xpath("profile", "fullname")
        )[0].text
    ...
    def follow_user(self):
        ...
        elem = self.driver.find_elements_by_xpath(
            xpath.read_xpath("profile", "following_button")
        )
        elem[0].click()
    ...
    def unfollow_user(self):
        ...
        """
        elem = self.driver.find_elements_by_xpath(
            xpath.read_xpath("profile", "unfollowing_button")
        )
        elem[0].click()
